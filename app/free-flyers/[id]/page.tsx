...
import { FreeFlyersLeadClient } from '@/components/free-flyers/FreeFlyersLeadClient';

export default async function FreeFlyersLeadPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <a href="https://photos4realestate.ca/" className="flex items-center gap-3" target="_blank" rel="noopener noreferrer">
          ...
          <a href="https://photos4realestate.ca/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center whitespace-nowrap rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 cursor-pointer">Home</a>
        </div>
      </header>

      <main className="mx-auto max-w-5xl p-4 md:p-8 space-y-8">
        <FreeFlyersLeadClient id={id} />
      </main>
      ...
      <footer className="mt-8 border-t border-white/10 bg-[#131c3b] text-gray-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 text-center text-sm">
          <div>
            Powered by{' '}
            <a href="https://photos4realestate.ca/" target="_blank" rel="noopener noreferrer" className="underline">Photos 4 Real Estate</a>{' '}· © {new Date().getFullYear()} All Rights Reserved
          </div>
