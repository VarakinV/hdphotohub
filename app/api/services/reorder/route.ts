...
export async function POST(req: NextRequest) {
  try {
    ...
    const whereScope = me.role === "SUPERADMIN" ? { id: { in: ids } } : { id: { in: ids }, adminId: me.id };
    const found = await prisma.service.findMany({ where: whereScope, select: { id: true } });
    const foundIds = new Set(found.map((f) => f.id));
    const filtered = ids.filter((id) => foundIds.has(id));
    if (!filtered.length) return NextResponse.json({ error: "No matching services" }, { status: 404 });
