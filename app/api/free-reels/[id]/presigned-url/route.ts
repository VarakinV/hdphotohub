...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const lead = await prisma.freeReelsLead.findUnique({ where: { id }, select: { id: true, status: true } });
    ...
    let basePath: string;
    if (category === 'headshot') basePath = `free-reels/${id}/headshot`;
    else if (category === 'logo') basePath = `free-reels/${id}/logo`;
    else basePath = `free-reels/${id}/images`;
    ...
    console.error('free-reels presigned-url failed', e);
