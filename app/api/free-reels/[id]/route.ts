...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    let lead = await prisma.freeReelsLead.findUnique({
      where: { id },
      include: {
        images: { orderBy: { sortOrder: 'asc' } },
        reels: { orderBy: { createdAt: 'asc' } },
      },
    });
    if (!lead) return NextResponse.json({ error: 'Not found' }, { status: 404 });

    // If webhook cannot reach dev, proactively sync any in-progress reels
    const inProgress = (lead.reels || []).filter((r) => r.status === 'QUEUED' || r.status === 'RENDERING');
      ...
      await Promise.all(
        inProgress.map(async (r) => {
          try {
            ...
            await prisma.freeReel.update({ where: { id: r.id }, data });
          } catch (e) {
            // ignore single reel sync failures
          }
        })
      );

      // If all reels are no longer in progress, mark lead complete
      const remaining = await prisma.freeReel.count({ where: { leadId: id, status: { in: ['QUEUED', 'RENDERING'] } } });
      if (remaining === 0) {
        await prisma.freeReelsLead.update({ where: { id }, data: { status: 'COMPLETE' } });
      }

      // Re-fetch with updated values
      lead = await prisma.freeReelsLead.findUnique({
        where: { id },
        include: {
          images: { orderBy: { sortOrder: 'asc' } },
          reels: { orderBy: { createdAt: 'asc' } },
    ...
    console.error('free-reels GET failed', e);
