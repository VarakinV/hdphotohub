...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const lead = await prisma.freeFlyersLead.findUnique({ where: { id }, select: { id: true, status: true } });
    ...
    let basePath: string;
    if (category === 'headshot') basePath = `free-flyers/${id}/headshot`;
    else if (category === 'logo') basePath = `free-flyers/${id}/logo`;
    else basePath = `free-flyers/${id}/images`;
    ...
    console.error('free-flyers presigned-url failed', e);
