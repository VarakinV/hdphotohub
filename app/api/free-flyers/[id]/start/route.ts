...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Render just the requested variant
    const basePath = `free-flyers/${id}`;
    try {
      // mark this variant as actively rendering
      await prisma.freeFlyer.updateMany({ where: { leadId: id, variantKey: variant }, data: { status: 'RENDERING', error: null } });
    ...
    const refreshed = await prisma.freeFlyersLead.findUnique({
      where: { id },
      include: { flyers: true },
    });
