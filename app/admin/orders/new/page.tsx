  ...
  useEffect(() => {
    (async () => {
      ...
      if (res.ok) {
        ...
        setRealtors(data.map((r: any) => ({ id: r.id, name: `${r.firstName} ${r.lastName}` })));
      }
    })();
  }, []);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormItem>
                ...
                <FormControl>
                  <Select value={field.value} onValueChange={field.onChange}>
                    <SelectTrigger className="w-full"><SelectValue placeholder="Select realtor" /></SelectTrigger>
                    <SelectContent>
                      {realtors.map((r) => (
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <textarea className="border rounded-md w-full min-h-24 p-3" value={field.value ?? ''} onChange={(e)=> field.onChange(e.target.value)} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <div className="flex justify-end gap-2">
            <Button type="button" variant="outline" onClick={() => router.push('/admin/orders')}>Cancel</Button>
            <Button type="submit" disabled={submitting}>
              {submitting ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin"/> Creating...</>) : 'Create Order'}
            </Button>
          </div>
        </form>
      </Form>
