...
export default function FreeSlideshowOrdersPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/admin/free-slideshow-orders?${q.toString()}`,
          { cache: 'no-store' }
        );
        const data = await res.json();
        setRows(data || []);
      } catch (e) {
        console.error('Failed to load free slideshow leads', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  async function onCopyLink(id: string) {
    try {
      ...
      await navigator.clipboard.writeText(`${origin}/free-slideshow/${id}`);
  ...
  async function onDeleteConfirmed() {
    ...
    try {
      setDeleting(true);
      const res = await fetch(`/api/admin/free-slideshow-orders/${deleteId}`, {
        method: 'DELETE',
      });
          ...
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                Free Slideshow Orders
              </h1>
              <p className="text-sm text-gray-600 mt-1">
                View and manage free slideshow submissions
              </p>
            </div>
          </div>
        </div>
