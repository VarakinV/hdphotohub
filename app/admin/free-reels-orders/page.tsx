...
export default function FreeReelsOrdersPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/admin/free-reels-orders?${q.toString()}`,
          { cache: 'no-store' }
        );
        const data = await res.json();
        setRows(data || []);
      } catch (e) {
        console.error('Failed to load free reels leads', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  async function onCopyLink(id: string) {
    try {
      ...
      await navigator.clipboard.writeText(`${origin}/free-reels/${id}`);
  ...
  async function onDeleteConfirmed() {
    ...
    try {
      setDeleting(true);
      const res = await fetch(`/api/admin/free-reels-orders/${deleteId}`, {
        method: 'DELETE',
      });
