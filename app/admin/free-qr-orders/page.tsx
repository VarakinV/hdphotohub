...
export default function FreeQrOrdersPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/admin/free-qr-orders?${q.toString()}`, {
          cache: 'no-store',
        });
        const data = await res.json();
        setRows(data || []);
      } catch (e) {
        console.error('Failed to load free QR leads', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  async function onCopyLink(id: string) {
    try {
      await navigator.clipboard.writeText(
        `${window.location.origin}/free-qr/${id}`
      );
  ...
  async function onDeleteConfirmed() {
    ...
    try {
      setDeleting(true);
      const res = await fetch(`/api/admin/free-qr-orders/${deleteId}`, {
        method: 'DELETE',
      });
            ...
            <>
              <Table className="table-fixed">
                ...
                <TableBody>
                  {pageItems.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell className="w-[170px] max-w-[170px] truncate">
