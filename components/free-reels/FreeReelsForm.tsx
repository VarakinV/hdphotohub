...
async function getPresigned(
  ...
  const res = await fetch(`/api/free-reels/${leadId}/presigned-url`, {
    method: 'POST',
...
export function FreeReelsForm() {
  ...
  // Auto-start upload whenever queued files exist and we have a leadId
  useEffect(() => {
      ...
      try {
        const cached =
          typeof window !== 'undefined'
            ? localStorage.getItem('free_reels_lead_id')
            : null;
        if (cached) {
          const res = await fetch(`/api/free-reels/${cached}`);
          if (res.ok) {
          ...
          localStorage.removeItem('free_reels_lead_id');
        }
      } catch {}

      const res = await fetch('/api/free-reels/prepare', { method: 'POST' });
      if (!res.ok) throw new Error('Failed to start session');
      const data = await res.json();
      setLeadId(data.id);
      try {
        localStorage.setItem('free_reels_lead_id', data.id);
      } catch {}
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/free-reels/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          leadId: id,
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="space-y-6">
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {/* in-dropzone overlay shows progress; no separate bar below */}
            <p className="text-gray-500 text-xs mt-1">
              Landscape ~400x150 px recommended
            </p>
            {!logoUrl && errors.logo ? (
          ...
          <div className="md:col-span-2">
            <Label className="mb-1 block">
              Property Photos (3-6) <span className="text-red-500">*</span>
            </Label>
            <input
              ref={inputRef}
              type="file"
            ...
            >
              <div className="text-sm text-gray-600 flex items-center gap-2">
                <Upload className="h-4 w-4" /> Drop images here or click to
                select
              </div>
            </div>
            ...
            {errors.images ? (
              ...
              <p className="text-gray-500 text-xs mt-1">
                Upload 3-6 photos (1920px+ long edge recommended)
              </p>
            )}
          </div>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="text-sm text-gray-600">
          We'll generate reels automatically and email you when they're ready.
        </div>
      </div>
    </form>
  );
