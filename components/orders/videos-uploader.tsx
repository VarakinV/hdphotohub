...
export function VideosUploader({ orderId, onUploaded }: Props) {
  ...
  function addFiles(fs: FileList | File[]) {
    const arr = Array.from(fs).filter((f) => f.type === 'video/mp4');
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {isConfigured
          ? 'S3 is configured. Drag and drop videos or click to browse.'
      ...
      >
        <div className="text-sm text-gray-600 flex items-center gap-2">
          <Upload className="h-4 w-4" /> Drop MP4 files here or click to select
        </div>
      </div>
