...
export function GenerateReelsButton({
  orderId,
  onStarted,
}: {
  orderId: string;
  ...
  async function handleGenerate() {
    try {
      setSubmitting(true);
      const res = await fetch(`/api/orders/${orderId}/reels/generate`, {
        method: 'POST',
      });
      if (!res.ok) throw new Error('Failed to start generation');
      // Notify parent immediately and optionally reload UI
      onStarted?.();
      await load();
    } catch (e) {
      // noop; could toast
    ...
    <div className="pt-2">
      ...
      >
        {submitting ? (
          ...
          <>
            <PlaySquare className="mr-2 h-4 w-4" /> Generate Reels
          </>
        )}
      </Button>
    </div>
